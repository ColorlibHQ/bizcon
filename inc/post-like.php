...
add_action( 'wp_enqueue_scripts', 'bizcon_sl_enqueue_scripts' );
function bizcon_sl_enqueue_scripts() {

	wp_enqueue_script( 'bizcon-post-likes', BIZCON_DIR_JS_URI . 'post-likes.js', array( 'bizcon-ui-js' ), '0.5-s2', true );
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_bizcon_process_simple_like', 'bizcon_process_simple_like' );
add_action( 'wp_ajax_bizcon_process_simple_like', 'bizcon_process_simple_like' );
function bizcon_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'bizcon' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'bizcon' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'bizcon' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'bizcon' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'bizcon' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'bizcon' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'bizcon_show_user_likes' );
add_action( 'edit_user_profile', 'bizcon_show_user_likes' );
function bizcon_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'bizcon' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'bizcon' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
