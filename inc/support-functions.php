...
// social media
if ( ! function_exists( 'bizcon_social' ) ) {
	function bizcon_social( $args = array()  ){
		...
		$url = bizcon_opt('bizcon_social_url');
...
function bizcon_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'bizcon' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'bizcon' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'bizcon' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function bizcon_set_post_views($postID) {
    $count_key = 'bizcon_post_views_count';
...
// blog post categoty 
function bizcon_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'bizcon' ) );
